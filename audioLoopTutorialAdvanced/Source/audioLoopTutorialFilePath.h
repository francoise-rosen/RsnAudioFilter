    ...
    void getNextAudioBlock (const AudioSourceChannelInfo& bufferToFill) override
    {
        ...
        while (outputSamplesRemaining > 0)
        {
            ...
            for(auto channel = 0; channel < numOutputChannels; ++channel)
            {
                ...
                // where do I apply gain, to in or out buffer?
                ...
                // apply gain (out buffer)
                // works but there're clicks on every start of the loop
                // those clicks somehow pass the closed slider
                bufferToFill.buffer->applyGain(channel,
                                               0,
                                               bufferToFill.numSamples, // not samplesThisTime! -> clicks
                                               (float)outputLevel);
